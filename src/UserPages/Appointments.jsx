...
const Appointments = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-gradient-to-r from-blue-100 to-purple-200">
      <div className="w-full max-w-2xl bg-white rounded-lg shadow-lg p-4 sm:p-6 lg:p-8">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-700 mb-4 sm:mb-6 text-center">My Appointments</h2>
        
        {error && <p className="text-red-500 text-center mb-4">{error}</p>}

        {loading ? (
          <div className="flex justify-center items-center h-32 sm:h-64">
            <span className="loading loading-spinner bg-purple-400 w-4 h-4 sm:w-6 sm:h-6 lg:w-8 lg:h-8"></span>
          </div>
        ) : (
          sortedAppointments.length > 0 ? (
            <ul className="space-y-4">
              {sortedAppointments.map((appointment) => (
                ...
                >
                  <div className="flex items-center mb-4 sm:mb-0 sm:mr-4">
                    {appointment.storeId?.images && (
                      <img
                        src={appointment.storeId.images[0]}
                        alt={appointment.storeId.storeName}
                        className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover mr-4"
                      />
                    )}
                    <div>
                  ...
                  <div className="flex-grow mb-4 sm:mb-0">
                    ...
                    {appointment.serviceIds.map((service, index) => (
                      <p key={index} className="text-xs sm:text-sm">{service.name} - {service.duration}</p>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 sm:mt-0">
                    {appointment.status !== "Cancelled" ? (
                      <button
                        onClick={() => updateAppointmentStatus(appointment._id, "Cancelled")}
                        className="px-3 py-1 text-sm font-semibold rounded-lg bg-red-400 hover:bg-red-600 text-white transition duration-200"
                      >
                        Cancel
                      </button>
                    ) : (
                      <button
                        onClick={() => bookAgain(appointment.storeId)}
                        className="px-3 py-1 text-sm font-semibold rounded-lg bg-green-400 hover:bg-green-600 text-white transition duration-200"
                      >
                        Book Again
                      </button>
                    )}
